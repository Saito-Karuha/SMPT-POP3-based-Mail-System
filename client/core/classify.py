...
def load_train_word_dict():
    with codecs.open("E:\six_courses\computer_network\homework\SMPT-POP3-based-Mail-System-master\client\core\\train_word_dict", "r", encoding="gbk", errors="ignore") as f:
        content = f.read()
    return content


def is_spam(email_content, train_word_dict, spam_count, ham_count):
    """
    判断邮件是否为垃圾邮件
    ...
    """
    # 假设 email_content 是邮件内容的字符串
    word_dict = create_word_dict(email_content, stop_words)  # 需要提前加载 stop_words
    result = predict_dataset(train_word_dict, spam_count, ham_count, {"word_dict": word_dict, "spam": None})
    return result == 1
...
if __name__ == '__main__':
    ...
    ham_count = train_set.spam.value_counts()[0]
    spam_count = train_set.spam.value_counts()[1]
